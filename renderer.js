...
const SERVER_URL = 'https://ghostchat-1-8vc2.onrender.com'; // Production URL
        ...
        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: calc(100% - 32px);">
            <div style="background: var(--bg-sidebar); padding: 40px; border-radius: 12px; width: 440px; box-shadow: 0 10px 30px rgba(0,0,0,0.3);">
                ...
                <div class="input-group" style="margin-bottom: 20px;">
                    ...
                    <input type="text" id="nickname-input" style="width: 100%; padding: 12px;" placeholder="Nasıl görünmek istersin?" value="${myNickname}">
                </div>
                
                <div class="input-group" style="margin-bottom: 32px;">
                    <label class="input-label">Oda Kodu</label>
                    <input type="text" id="room-input" style="width: 100%; padding: 12px;" placeholder="Oda kodu girin..." value="${myRoomId}">
                </div>
...
async function initApp() {
    ...
    socket.on('muted-by-owner', () => {
        if (!isMuted) {
            toggleMute();
            alert('Oda sahibi tarafından susturuldunuz.');
        }
    });

    // Load message history from cache
    const cachedMessages = JSON.parse(localStorage.getItem(`msgs_${myRoomId}`) || '[]');
...
function loadSettings() {
    const settings = JSON.parse(localStorage.getItem('ghostchat_settings') || '{}');
    if (settings.nickname) myNickname = settings.nickname;
    if (settings.roomId) myRoomId = settings.roomId;
    if (settings.selectedInput) selectedInput = settings.selectedInput;
    if (settings.selectedOutput) selectedOutput = settings.selectedOutput;
    if (settings.isNoiseSuppressionEnabled !== undefined) isNoiseSuppressionEnabled = settings.isNoiseSuppressionEnabled;
}
               ...
               `;
                ...
                userItem.oncontextmenu = (e) => {
                    ...
                    if (socket.id === ownerId && u.id !== socket.id) {
                        showContextMenu(e.clientX, e.clientY, [
                            { label: 'Sustur', action: () => socket.emit('mute-user', { roomId: myRoomId, targetId: u.id }) },
